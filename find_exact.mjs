       ...
       for(let i=0; i<Math.min(sz-32, 200); i++) {
           try {
               ...
               if (h > 0 && h < 200) {
                   const pBak = s.p;
                   const vP = s.rP();
                   if (vP >= 0 && vP < 1000000) allChStats[ch].p[h] = Math.max(allChStats[ch].p[h]||0, vP);
...
const req = [242, 496, 207, 42, 5, 210, 146, 31, 13];
for (const [ch, d] of Object.entries(allChStats)) {
    const vals = new Set([...Object.values(d.p), ...Object.values(d.b11)]);
    const matched = req.filter(x => vals.has(x));
    if (matched.length >= 4) {
        console.log(`CH ${ch} matched ${matched.length}: ${matched.join(', ')}`);
        for (const [h, v] of Object.entries(d.p)) if (req.includes(v)) console.log(`  Handle ${h} (packed) = ${v}`);
        for (const [h, v] of Object.entries(d.b11)) if (req.includes(v)) console.log(`  Handle ${h} (bits11) = ${v}`);
    }
}
