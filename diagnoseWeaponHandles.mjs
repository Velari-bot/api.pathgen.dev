...
// Basic decryption and chunk reading
let posKey = null;
...
for (const [ch, props] of Object.entries(allChStats)) {
    // looking for 92 or 12 or 4 or 21
    const vals = Object.values(props);
    if (vals.includes(92) && vals.includes(12) && vals.includes(4)) {
        console.log(`POTENTIAL SHOTGUN CHANNEL: ${ch}`, props);
    }
    if (vals.includes(21) && vals.includes(2)) {
        console.log(`POTENTIAL AR CHANNEL: ${ch}`, props);
    }
}
