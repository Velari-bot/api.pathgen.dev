...
// Decryption keys found in previous steps
let posKey = null;
...
while(b1.p < allRaw.length*8 - 128) {
    ...
    if (sz > 8 && t === 1) {
        ...
        // Look for string anywhere in packet
        for (let j=0; j<bytes.length-10; j++) {
            const l = bytes.readInt32LE(j);
            if (l > 5 && l < 100) {
                const s = bytes.slice(j+4, j+4+l-1).toString();
                if (s.startsWith('WID_') || s.startsWith('Item_') || s.startsWith('B_')) {
                    if (!chNames[ch]) chNames[ch] = new Set();
                    chNames[ch].add(s);
                }
            }
        }
...
for (const ch in chNames) {
    console.log(`CH ${ch}: ${Array.from(chNames[ch]).join(', ')}`);
}
