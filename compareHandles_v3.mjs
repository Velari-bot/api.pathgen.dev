        ...
        if (sz > 16 && t === 1 && ch >= 2 && ch <= 60000) {
            ...
            for (let j = 0; j < bytes.length - 12; j++) {
                ...
                if (l > 5 && l < 100) {
                    ...
                    if (s.includes('WID_') || s.startsWith('B_')) weaponNames[ch] = s;
                }
            }
            for(let i=0; i<Math.min(sz-32, 400); i++) {
                try {
                    ...
                    if (h > 0 && h < 400) {
                        const v = s.rP();
                        if (v >= 0 && v <= 10000) allChStats[ch][h] = Math.max(allChStats[ch][h]||0, v);
                    }
                } catch(x){}
            }
        }
...
(async () => {
    ...
    for (const ch in n.allChStats) {
        ...
        if (p[1] === 12 || p[64] === 92 || p[113] === 4) {
             console.log(`CH ${ch} [${n.weaponNames[ch]}]: 1=${p[1]}, 64=${p[64]}, 113=${p[113]}`);
        }
    }
    ...
    for (const ch in o.allChStats) {
        ...
        if (p[11] === 18 || p[21] === 328 || p[13] === 8) {
             console.log(`CH ${ch} [${o.weaponNames[ch]}]: 11=${p[11]}, 21=${p[21]}, 13=${p[13]}`);
        }
    }
})();
