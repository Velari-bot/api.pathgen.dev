...
router.get('/cosmetics', validateFirestoreKey(1), async (req, res) => {
    try {
        const data = await fortniteLib.getBRCosmetics();
        res.json({ status: 200, credits_used: 1, data });
    } catch(err) {
        res.status(500).json({ status: 500, error: 'Could not fetch cosmetics' });
    }
});

router.get('/cosmetics/new', validateFirestoreKey(1), async (req, res) => {
    try {
...
router.get('/cosmetics/search', validateFirestoreKey(1), async (req, res) => {
...
router.get('/cosmetics/:id', validateFirestoreKey(1), async (req, res) => {
    try {
...
router.get('/shop', validateFirestoreKey(1), async (req, res) => {
    try {
        ...
        res.json({ status: 200, credits_used: 1, data: finalResult });
    } catch(err) {
        res.status(500).json({ status: 500, error: 'Shop fetch and merge failed' });
    }
});

router.get('/weapons', validateFirestoreKey(1), async (req, res) => {
    res.json({ status: 200, credits_used: 1, data: { pool: ['Assault Rifle', 'Shotgun', 'Sniper', 'Pistol', 'SMG'] } });
});
...
router.get('/news', validateFirestoreKey(1), async (req, res) => {
    try {
...
router.get('/playlists/:id', validateFirestoreKey(1), async (req, res) => {
    try {
 ...
 */
router.get('/creator/:name', validateFirestoreKey(1), async (req, res) => {
    try {
 ...
 * Banner Assets
 */
router.get('/banners', validateFirestoreKey(1), async (req, res) => {
    try {
...
router.get('/banners/colors', validateFirestoreKey(1), async (req, res) => {
    try {
 ...
 */
router.get('/cosmetics/br', validateFirestoreKey(1), async (req, res) => {
    try {
        const data = await fortniteLib.getBRCosmetics();
        res.json({ status: 200, credits_used: 1, data });
    } catch(err) {
...
router.get('/cosmetics/tracks', validateFirestoreKey(1), async (req, res) => {
    try {
...
router.get('/cosmetics/instruments', validateFirestoreKey(1), async (req, res) => {
    try {
...
router.get('/cosmetics/cars', validateFirestoreKey(1), async (req, res) => {
    try {
...
router.get('/cosmetics/lego', validateFirestoreKey(1), async (req, res) => {
    try {
...
router.get('/cosmetics/lego/kits', validateFirestoreKey(1), async (req, res) => {
    try {
...
router.get('/cosmetics/beans', validateFirestoreKey(1), async (req, res) => {
    try {
 ...
 */
router.get('/cosmetics/search/all', validateFirestoreKey(2), async (req, res) => {
    try {
        const data = await fortniteLib.searchCosmeticsAll(req.query);
        res.json({ status: 200, credits_used: 2, data });
    } catch(err) {
        res.status(500).json({ status: 500, error: 'Advanced search failed' });
    }
});

router.get('/cosmetics/search/ids', validateFirestoreKey(2), async (req, res) => {
    const { ids } = req.query;
 ...
 */
router.get('/news/br', validateFirestoreKey(1), async (req, res) => {
    try {
...
router.get('/news/stw', validateFirestoreKey(1), async (req, res) => {
    try {
...
router.get('/news/creative', validateFirestoreKey(1), async (req, res) => {
    try {
 ...
 */
router.get('/accounts/bulk', validateFirestoreKey(5), async (req, res) => {
    const { ids } = req.query;
    if (!ids) return res.status(400).json({ error: 'Missing ids' });
    try {
        const data = await osirion.lookupByAccountIdBulk(ids.split(','));
        res.json({ status: 200, credits_used: 5, data });
    } catch(err) {
...
router.get('/cosmetics/sets', validateFirestoreKey(1), async (req, res) => {
    try {
 ...
 */
router.get('/tournaments', validateFirestoreKey(2), async (req, res) => {
...
router.get('/tournaments/leaderboard', validateFirestoreKey(5), async (req, res) => {
 ...
 */
router.get('/discovery', validateFirestoreKey(1), async (req, res) => {
...
router.get('/discovery/page', validateFirestoreKey(1), async (req, res) => {
...
router.get('/mnemonic/bulk', validateFirestoreKey(5), async (req, res) => {
...
router.get('/mnemonic/:id', validateFirestoreKey(2), async (req, res) => {
    try {
 ...
 */
router.get('/aes', validateFirestoreKey(1), async (req, res) => {
    try {
...
// Version 1 Free
router.get('/lookup', validateFirestoreKey(2), async (req, res) => {
  ...
  try {
    ...
    res.json({ ...stats, credits_used: 2 });
  } catch(err) {
    res.status(500).json({ error: 'Lookup failed' });
  }
});

router.get('/ranked', validateFirestoreKey(5), async (req, res) => {
  const { name, accountId } = req.query;
  if (!name && !accountId) return res.status(400).json({ error: 'Missing name or accountId parameter' });
  ...
  try {
    ...
    // Final Mirroring check for R2 storage
    const finalResult = await mirrorObjectUrls(fused);
    res.json({ ...finalResult, credits_used: 5 });
  } catch(err) {
    res.status(500).json({ error: 'Ranked reconcile failed' });
  }
});

router.get('/stats', validateFirestoreKey(5), async (req, res) => {
  ...
  try {
    ...
    res.json({ ...merged, credits_used: 5 });
  } catch(err) {
    res.status(500).json({ error: 'Stats fetch and merge failed' });
  }
});

router.get('/ranked/modes', validateFirestoreKey(1), async (req, res) => {
    try {
 ...
 */
router.get('/stats/br/v2', validateFirestoreKey(2), async (req, res) => {
...
router.get('/stats/br/v2/:accountId', validateFirestoreKey(2), async (req, res) => {
 ...
 */
router.get('/playlists', validateFirestoreKey(1), async (req, res) => {
    try {
