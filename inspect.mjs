...
const reqChannels = [24, 160, 5364, 4103, 5635, 5838];
for (const tc of reqChannels) {
    const d = { p: {}, b11: {} };
           ...
           for(let i=0; i<Math.min(sz-32, 200); i++) {
               try {
                   ...
                   if (h > 0 && h < 200) {
                       const pBak = s.p;
                       const vP = s.rP();
                       if (vP >= 0 && vP < 1000000) d.p[h] = Math.max(d.p[h]||0, vP);
                       s.p = pBak; const v11 = s.rBs(11);
                       if (v11 >= 0 && v11 <= 2047) d.b11[h] = Math.max(d.b11[h]||0, v11);
                   }
               } catch(x){}
           }
       }
    ...
    console.log(`\n\n=== CH ${tc} ===`);
    console.log("Packed: ", JSON.stringify(d.p));
    console.log("Bits11: ", JSON.stringify(d.b11));
}
