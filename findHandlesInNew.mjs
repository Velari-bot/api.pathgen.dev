    ...
    if (sz > 16 && t === 1 && ch >= 2 && ch <= 60000) {
        ...
        for (let j = 0; j < bytes.length - 12; j++) {
            ...
            if (l > 5 && l < 100) {
                ...
                if (s.startsWith('WID_') || s.startsWith('Item_')) {
                    weaponNames[ch] = s;
                }
            }
        }
...
console.log("CHANNELS WITH WEAPON NAMES:");
for (const ch in weaponNames) {
    console.log(`CH ${ch}: ${weaponNames[ch]}`);
    console.log(`  Props: ${JSON.stringify(allChStats[ch])}`);
}
