...
for (let i = 400; i < 1500; i++) {
  if (buf.readUInt32LE(i) === 32 && buf.readUInt32LE(i+36) < 10 && buf.readUInt32LE(i + 40) > 0) {
    ...
    if (sz > 16 && t === 1 && ch >= 2 && ch <= 60000) {
        ...
        for(let i=0; i<Math.min(sz-32, 400); i++) {
            try {
                ...
                if (h > 0 && h < 200) {
                    const v = s.rP();
                    if (v >= 0 && v <= 50000) allChStats[ch][h] = v;
                }
            } catch(x){}
        }
    }
...
for (const [ch, props] of Object.entries(allChStats)) {
    ...
    const hasName = strings[ch];
    
    // Shotgun: shots 12, hits 4, damage 92
    if (vals.includes(12) && vals.includes(4) && vals.includes(92)) {
        console.log(`SHOTGUN CANDIDATE: CH ${ch} [${hasName}]`, props);
    }
    // AR: shots 21, headshots 2
    if (vals.includes(21) && vals.includes(2) && !vals.includes(12)) {
         console.log(`AR CANDIDATE: CH ${ch} [${hasName}]`, props);
    }
    // Revolver: shots 4
    if (vals.includes(4) && !vals.includes(12) && !vals.includes(21)) {
         // This might be too frequent, let's filter by name if possible
         if (hasName && (hasName.includes('Revolver') || hasName.includes('Handgun'))) {
             console.log(`REVOLVER CANDIDATE: CH ${ch} [${hasName}]`, props);
         }
    }
    // Boomstick: equips 5? 
    if (hasName && hasName.includes('Boomstick')) {
        console.log(`BOOMSTICK CANDIDATE: CH ${ch} [${hasName}]`, props);
    }
}
